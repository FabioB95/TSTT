...
from model_benchmarkpt1 import model
import pandas as pd
import json
...
with open("output_vehicle_on_arc_DEF_benchmark.csv", "w") as f:
...
# --- Salvataggio CSV ---
pd.DataFrame(assignments).to_csv("output_y_DEF_benchmark.csv", index=False)
pd.DataFrame(flows).to_csv("output_x_DEF_benchmark.csv", index=False)
pd.DataFrame(tti_estimates).to_csv("output_sigma_DEF_benchmark.csv", index=False)

print("\n📁 Risultati salvati in: output_y_DEF.csv, output_x_DEF.csv, output_sigma_DEF.csv")

# --- Lambda ---
with open("output_lambda_DEF_benchmark.csv", "w") as f:
