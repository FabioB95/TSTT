...
from model_minimizza_TTI import model
import pandas as pd
import json
import os
...
with open("output_vehicle_on_arc_minimizza_TTI.csv", "w") as f:
...
# --- Salvataggio CSV ---
pd.DataFrame(assignments).to_csv(f"output_y_DEF_alpha_minimizza_TTI_{ALPHA}.csv", index=False)
pd.DataFrame(flows).to_csv("output_x_minimizza_TTI.csv", index=False)
pd.DataFrame(tti_estimates).to_csv("output_sigma_minimizza_TTI.csv", index=False)

print("\n📁 Risultati salvati in: output_y_DEF.csv, output_x_DEF.csv, output_sigma_DEF.csv")

# --- Lambda ---
with open("output_lambda_minimizza_TTI.csv", "w") as f:
    f.write("arc,time,h,lambda\n")
    for (i, j, t, h) in model.ATH:
        val = model.lmbda[i, j, t, h].value
        if val and val > 0.001:
            f.write(f"{i}->{j},{t},{h},{val}\n")
