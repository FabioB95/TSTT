...
#with open("dati/c_cost_DEF_15minuti.json") as f:
#    c_cost_data = json.load(f)

#with open("dati/c_cost_FP_DEF_15minuti.json") as f:
#    c_cost_FF = json.load(f)
...
def fftt_c_rule(m, c):
    return COST_FF.get(c, 1)  # fallback = 1 se non trovato

model.fftt_c = Param(model.C, initialize=fftt_c_rule)
...
model.overload = Var(model.A * model.T, domain=NonNegativeReals)

def arc_capacity_soft(m, i, j, t):
    return m.x[i, j, t] <= m.mu[i, j] + m.overload[i, j, t]

model.soft_cap = Constraint(model.A * model.T, rule=arc_capacity_soft)
...
ALPHA = float(os.getenv("ALPHA", 2.0)) 
...
epsilon = 0.0001
model.obj = Objective(
    expr=sum(model.sigma[i, j, t] for (i, j) in model.A for t in model.T),
    sense=minimize
)
